...
namespace hlab {
	...
	bool Ex2001_GamePlay::InitScene() {
		...
		m_sound = std::make_unique<DirectX::SoundEffect>(
			m_audEngine.get(),
			L"../Assets/Sound/"
			L"63272__robinhood76__ad003jinglebellsoneshot-230715-021210.wav");
	...
	void Ex2001_GamePlay::Update(float dt) {
		...
		if (state == 0) { // 정지 상태.
			...
			// Control 키를 떼는 순간 즉시 애니메이션 상태 변화.
			static bool prevSpace = false;
			if (AppBase::m_keyPressed[VK_CONTROL] == true) {
				prevSpace = true;
			}
			if (prevSpace && AppBase::m_keyPressed[VK_CONTROL] == false) {
				state = 1;
				frameCount = 0;
				prevSpace = false;
			}
					 ...
					 AppBase::m_keyPressed[VK_CONTROL]) {
			...
			if (frameCount == 115) {
				...
				Matrix temp = m_character->m_worldRow;
				...
				dir = Vector4::Transform(
					...
					m_character->m_aniData.accumulatedRootTransform);
				dir.Normalize(); // worldRow에 스케일이 있어서 작아진 벡터 키우기.
				dir *= 1.5f / m_simToRenderScale; // 속력.

				CreateDynamic(PxTransform(PxVec3(handPos.x, handPos.y, handPos.z) /
										  m_simToRenderScale),
							  PxSphereGeometry(5.0f), PxVec3(dir.x, dir.y, dir.z));
			}
